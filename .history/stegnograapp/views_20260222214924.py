...
def login(request):
    ...
    # Check if they just arrived from a successful registration
...
def register(request):
    if request.method == 'POST':
        ...
        if user.objects.filter(email=email).exists():
            ...
            try:
                subject = 'PixelSafe - Verify Your Registration'
...
def verify_register_otp(request):
    ...
    if request.method == 'POST':
        ...
        if user_entered_otp == actual_otp:
            ...
            # Trigger the success modal on the login page
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            try:
                ...
                print(f"Error sending email: {e}")
                context['error'] = "Failed to send OTP email. Please check your email configuration."
        ...
        try:
            ...
            if action == 'encode':
                ...
                try:
                    ...
                    # Save the user with the upload log
                    log_entry = upload(media=carrier, activity_type=desc, user=active_user)
                    log_entry.save()
                    carrier.seek(0)
                except Exception as e:
            ...
            # ================= DECODING =================
                ...
                try:
                    ...
                    # Save the user with the upload log
                    log_entry = upload(stego_media=stego_file, activity_type=desc, user=active_user)
                    log_entry.save()
                    stego_file.seek(0)
                except Exception: pass
...
# --- NEW RECENT ACTIVITIES VIEW ---
def recent_activities(request):
    # Security check: Make sure an admin is logged in
    if request.session.get('email') != "admin@bca":
        return redirect('adminlogin')
        
    # Fetch all uploads (activities), ordered by most recent first
    activities = upload.objects.all().order_by('-uploaded_at')
    
    # Calculate some quick stats for the sidebar
    active_users = user.objects.count()
    total_encoded = upload.objects.filter(activity_type__startswith='Encoded').count()
    total_decoded = upload.objects.filter(activity_type__startswith='Decoded').count()
