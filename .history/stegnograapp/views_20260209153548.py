...
from .models import user, detail, upload  # Import upload model
...
def main(request):
    ...
    if 'email' in request.session:
        try:
            context['user'] = user.objects.get(email=request.session['email'])
        except: pass
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if action == 'encode':
                ...
                # --- LOG ACTIVITY: Save to Upload Model ---
                try:
                    # We save a copy of the input file to the history log
                    # Use a new instance to avoid file pointer conflicts
                    log_entry = upload(media=carrier)
                    log_entry.save()
                ...
                # --- 2. CARRIER: AUDIO ---
                    ...
                    try:
                        if steg_mode == 'text-in-audio':
                            ...
                            if steg_mode == 'image-in-audio': d.encode_image_in_audio_number = inc(d.encode_image_in_audio_number)
                            elif steg_mode == 'audio-in-audio': d.encode_audio_in_audio_number = inc(d.encode_audio_in_audio_number)
                            elif steg_mode == 'video-in-audio': d.encode_video_in_audio_number = inc(d.encode_video_in_audio_number)
                ...
                # --- 3. CARRIER: VIDEO ---
                    ...
                    try:
                        if steg_mode == 'text-in-video':
                            ...
                            if steg_mode == 'image-in-video': d.encode_image_in_video_number = inc(d.encode_image_in_video_number)
                            elif steg_mode == 'audio-in-video': d.encode_audio_in_video_number = inc(d.encode_audio_in_video_number)
                            elif steg_mode == 'video-in-video': d.encode_video_in_video_number = inc(d.encode_video_in_video_number)
            ...
            # ================= DECODING =================
                ...
                # --- LOG ACTIVITY ---
                try:
                    log_entry = upload(stego_media=stego_file)
                    log_entry.save()
                    stego_file.seek(0)
                except Exception: pass
                ...
                if result_data is not None:
                    if 'text' in steg_mode:
                        ...
                        # Update Decode Stats
                        d.decode_image_in_image_number = inc(d.decode_image_in_image_number) # Generic increment for file decode
                        d.save()
